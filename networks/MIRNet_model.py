...
class RRG_xiaotian(nn.Module):
    def __init__(self, n_feat, n_MSRB, height, width, stride, bias=False):
        super(RRG_xiaotian, self).__init__()
        modules_body = [MSRB(n_feat, height, width, stride, bias) for _ in range(n_MSRB)]
        modules_body.append(conv(n_feat, n_feat, kernel_size=3))
        self.body = nn.Sequential(*modules_body)
...
class MIRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.conv_out(h)
        h += x
        return torch.clamp(h, min = -1,max = 1)
    # def unfreeze(self):
...
####    xiotian model
class MIRNet_xiaotian(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=64, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False):
        super(MIRNet, self).__init__()

        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)

        modules_body = [RRG_xiaotian(n_feat, n_MSRB, height, width, stride, bias) for _ in range(n_RRG)]
        self.body = nn.Sequential(*modules_body)
...
class MIRNet_fusion(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=64, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False):
        super(MIRNet_fusion, self).__init__()

        self.conv_in_blur = nn.Conv2d(in_channels, n_feat//2, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)
        self.conv_in_dark = nn.Conv2d(in_channels, n_feat//2, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)
    ...
    def forward(self, blur,dark):
        blur = self.conv_in_blur(blur)
        dark = self.conv_in_blur(dark)
        h = torch.cat([blur,dark],dim=1)
        h = self.body(h)
        h = self.conv_out(h)
        #h += x
        return h
